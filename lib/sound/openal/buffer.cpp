...
namespace OpenAL
{
    ...
    void Buffer::bufferData(const Sound::DataBuffer& data)
    {
        ...
        if (data.channelCount() == 1 && data.bytesPerSample() == 1)
            format = AL_FORMAT_MONO8;
        else if (data.channelCount() == 1 && data.bytesPerSample() == 2)
            format = AL_FORMAT_MONO16;
        else if (data.channelCount() == 2 && data.bytesPerSample() == 1)
            format = AL_FORMAT_STEREO8;
        else if (data.channelCount() == 2 && data.bytesPerSample() == 2)
            format = AL_FORMAT_STEREO16;
        else
            throw std::runtime_error("OpenAL::Buffer.bufferData: cannot handle given combination of channel count and bytes/sample");

        alBufferData(buffer, format, data, data.size(), data.frequency());
        ...
        _duration = float(samplecount) / float(data.frequency());
    }
