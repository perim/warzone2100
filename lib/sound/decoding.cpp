...
namespace Sound
{
        ...
        _input(input)
    {
        ...
        if (error < 0)
            throw std::runtime_error("Sound::Decoding: VorbisFile returned an error while opening; errorcode: " + lexical_cast<string>(error));
    ...
    void Decoding::reset()
    {
        ...
        int error = ov_open_callbacks(this, &_oggVorbisStream, NULL, 0, oggVorbis_callbacks);
        if (error < 0)
            throw std::runtime_error("Sound::Decoding.reset: VorbisFile returned an error while opening; errorcode: " + lexical_cast<string>(error));
        ...
        while(size < bufferSize)
        {
            ...
            if(result > 0)
                ...
                if(result < 0)
                    ...
                    throw std::runtime_error("Sound::Decoding.decode: error decoding OggVorbis stream; errorcode: " + lexical_cast<string>(result) + " with buffersize: " + lexical_cast<string>(bufferSize));
                else
                    break;
        }
